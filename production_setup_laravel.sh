...
,s/^DB_DATABASE.*$/DB_DATABASE=${MY_DATABASE_NAME}/g
...
if [ ! -d /usr/share/nginx ]; then 
  ...
  if [ -e /usr/share/nginx/html/todolist ]; then 
    sudo rm /usr/share/nginx/html/todolist
  fi
  sudo ln -s $(pwd)/public /usr/share/nginx/html/todolist
fi
